...
def save_time_period_pm(context, time_key, start_time, end_time):
    ...
    time_period[get_time_period_key(start_time, end_time)] = [start_time, end_time]
...
def get_inline_kb_for_change_time():
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("+", callback_data="TIME_+"),
                InlineKeyboardButton("-", callback_data="TIME_-"),
                InlineKeyboardButton("+", callback_data="TIME_+2"),
                InlineKeyboardButton("-", callback_data="TIME_-2"),
            ],
            [InlineKeyboardButton("📄 Сохранить", callback_data="TIME_SAVE")],
        ]
    )
...
def show_period_pm(update: Update, context: CallbackContext):
    ...
    reply_keyboard = list(keyboard_row_divider(get_array_time_period(context), 1))
...
def change_or_add_period_pm(update: Update, context: CallbackContext):
    ...
    if update.message.text == "Добавить":
        reply_keyboard = list(keyboard_row_divider(["Утро", "Вечер"], 2))
        ...
        reply_keyboard = list(keyboard_row_divider(["Изменить", "Удалить"]))
...
def edit_period_pm(update: Update, context: CallbackContext):
    ...
    try:
        ...
        context.user_data["end_time"] = datetime(1970, 1, 1, end_hours, end_minutes, 0)
        start_time = context.user_data["start_time"]
        end_time = context.user_data["end_time"]
...
def inline_button_change_time(update: Update, context: CallbackContext):
    ...
    if query.data == "TIME_+":
        ...
        save_time_period_pm(
            context, context.user_data["time_key"], start_time, end_time
        )
        return show_period_pm(update, context)
...
pm_conv = ConversationHandler(
    entry_points=[CallbackQueryHandler(show_period_pm, pattern="^show_period_pm")],
    states={
        "show_period_pm": [
            MessageHandler(Filters.text & ~Filters.command, show_period_pm)
        ],
        "change_or_add_period_pm": [
            MessageHandler(Filters.text & ~Filters.command, change_or_add_period_pm)
        ],
        "add_period_pm": [
            MessageHandler(Filters.text & ~Filters.command, add_period_pm)
        ],
        "edit_period_pm": [
            MessageHandler(Filters.text & ~Filters.command, edit_period_pm)
        ],
        "inline_button_change_time": [
            CallbackQueryHandler(inline_button_change_time, pattern="^TIME_")
        ],
        "question_change_or_delete_period": [
            MessageHandler(
                Filters.text & ~Filters.command, question_change_or_delete_period
            )
        ],
    },
    fallbacks=[
